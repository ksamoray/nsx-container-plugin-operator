...
faq -f yaml -o yaml --slurp \
    '.[0].spec.install = {strategy: "deployment", spec:{ deployments: [{name: .[1].metadata.name, spec: .[1].spec }], permissions: [{serviceAccountName: .[3].metadata.name, rules: .[2].rules }]}} | .[0]' \
...
faq -f yaml -o yaml --slurp \
    '.[0].metadata.annotations.containerImage = "'$IMG_HASHED'" | .[0].spec.install.spec.deployments[0].spec.template.spec.containers[0].image = "'$IMG_HASHED'" | .[0]'  ../../bundle/openshift4/manifests/nsx-container-plugin-operator.clusterserviceversion.yaml > csv.tmp.yaml &&
    cat csv.tmp.yaml > ../../bundle/openshift4/manifests/nsx-container-plugin-operator.clusterserviceversion.yaml && rm csv.tmp.yaml
